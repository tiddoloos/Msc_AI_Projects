...
def param_experiment(function, MU, TOUR_SIZE, NGEN, CXPB, MUTPB):
    results = []
    for j in range(1,TOUR_SIZE+1):
        print('TOUR_SIZE', j)
        toolbox.register("select", tools.selTournament, tournsize=j)
        print('MU: ', MU, 'TOURSIZE: ', j)
        pop, logbook, hof = function(MU, CXPB, MUTPB, NGEN)
        results.append([logbook, hof, MU, j, NGEN])
...
def save_results_df(results):
    ...
    df.to_csv('param_results/param_plus_toursize_1-10.csv')
...
def plot_save_results(results):
    ...
    for i in range(len(results)):
        ...
        MU_values.append(results[i][2])
        avg_results.append(results[i][0][NGEN-1]['avg'])
    if os.path.exists("param_results/ass2_param_tuning_gridplus_TOUR_SIZE_1-10.png"):
        print('removed')
        os.remove("param_results/ass2_param_tuning_gridplus_TOUR_SIZE_1-10.png")
    ...
    # os.remove('')
    plt.savefig('param_results/ass2_param_tuning_gridplus_TOUR_SIZE_1-10')
    # plt.show()
    ...
    f = open("param_results/params_MplusL_Toursize_1-10.txt","w+")
